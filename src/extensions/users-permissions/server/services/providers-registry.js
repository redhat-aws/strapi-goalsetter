...
const getInitialProviders = ({ purest }) => ({
  ...
  async keycloak({ access_token, providers }) {
    console.log('keycloak provider access_token: ' + access_token);
    const keycloak = purest({ provider: 'keycloak' });
    console.log('after purest: ' + keycloak);
    return keycloak
      ...
      .then(({ body }) => {
        ...
        if (!username || !email) {
          strapi.log.warn(
            'Keycloak Response Body did not contain required attributes: ' + JSON.stringify(body)
          );
        }
        return {
          username,
...
module.exports = () => {
  ...
  return {
    register(providerName, provider) {
      console.log('register providers-registry');
